...
import json
import time
import src.data.feature_store as feature_store
import src.model.train as train
import src.model.save as save_model
import src.data.explore_data as explore_data
...
def main():
    ...
    features_df = feature_store.cria_feature_store()
    ...
    explore_data.analisa_dados(features_df)

    # Chama a função para treinar e avaliar o modelo, retornando várias informações relevantes
    modelo, X_teste, y_teste, previsoes, acuracia, class_report = train.treina_avalia_modelo(features_df)
    ...
    save_model.salva_modelo(modelo, caminho_modelo)
    
    # Salva as previsões e os valores reais de teste em um arquivo CSV
    save_model.salva_previsoes(previsoes, y_teste, caminho_previsoes)
    ...
    save_model.salva_info(pipeline_run_info, pipeline_run_info_path)
